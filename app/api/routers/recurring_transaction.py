...
async def get_recurring_transaction(recurring_transaction_id: int,
                                    ...
                                    db: AsyncSession = Depends(get_db)):
    try:
        return await RecurringTransactionRepository.get_recurring_transaction(db, recurring_transaction_id=recurring_transaction_id, user_id=user.id)
...
async def create_recurring_transaction(recurring_transaction: RecurringTransactionCreate,
                                       ...
                                       db: AsyncSession = Depends(get_db)):
    try:
        return await RecurringTransactionRepository.create_recurring_transaction(db, recurring_transaction, user_id=user.id)
...
async def update_recurring_transaction(recurring_transaction_id: int,
                                       ...
                                       db: AsyncSession = Depends(get_db)):
    try:
        updated_recurring_transaction = await RecurringTransactionRepository.update_transaction(db, recurring_transaction_id, recurring_transaction_update, user_id=user.id)
